...
export default function DeviceFeature() {
  ...
  return (
    <section id="what-is-it" className="relative pt-0 pb-12 bg-white text-gray-900 overflow-x-hidden">
          ...
          {tabs.map((tab) => (
            <div key={tab.id} className="w-1/2">
              <Image
                ...
                alt={tab.id === "ipad" ? "Pop Quiz running on iPad" : "Pop Quiz running on iPhone"}
                width={tab.id === "ipad" ? 1970 : 2049}
                height={tab.id === "ipad" ? 1012 : 1055}
                className="w-full h-auto block"
                priority={tab.id === "ipad"}
              />
            </div>
          ))}
        </div>
      ...
      {/* Segmented control */}
      <div className="flex justify-center mt-6 mb-8" role="tablist" aria-label="Device view">
      ...
      <div key={activeTab} className="text-center max-w-4xl mx-auto px-6 animate-fade-in" role="tabpanel">
